...
wfl_internal_zscores_female<- blood_child_complete %>% 
  filter(bw_sex == 2) %>% 
  mutate(zwfl_internal_del_merge = (weight_del_merge/(1000*length_del_merge) - mean(weight_del_merge/(1000*length_del_merge)))/sd(weight_del_merge/(1000*length_del_merge)),
...
wfl_internal_zscores_male<- blood_child_complete %>% 
  filter(bw_sex == 1) %>% 
  mutate(zwfl_internal_del_merge = (weight_del_merge/(1000*length_del_merge) - mean(weight_del_merge/(1000*length_del_merge)))/sd(weight_del_merge/(1000*length_del_merge)),
            zwfl_internal_wk3 = (weight_wk3/(1000*length_wk3) - mean(weight_wk3/(1000*length_wk3)))/sd(weight_wk3/(1000*length_wk3)),
            zwfl_internal_wk6 = (weight_wk6/(1000*length_wk6) - mean(weight_wk6/(1000*length_wk6)))/sd(weight_wk6/(1000*length_wk6)),
...
# write_csv(blood_child_who_stan_complete, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Documents/Projects/S-PRESTO/input/anthropometry/blood_child_who_stan_complete_dropwk1_06062023.csv")
