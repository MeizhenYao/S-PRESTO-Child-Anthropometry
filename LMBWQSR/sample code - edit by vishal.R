...
for(i in 1:N) {
  ...
  c[i,1] ~ dnorm(c.mean[i,1], c.tau) # is c.tau need to be different?
  ...
  c.mean[i,1] <- inter1 + (gamma1*(X[i,1:P] %*% What1[1:P]))*sw[i] # what should I do if I want to use visit1 as my reference group?
  c.mean[i,2] <- inter2 + (gamma2*(X[i,1:P] %*% What2[1:P]))*sw[i] 
  c.mean[i,3] <- inter3 + (gamma3*(X[i,1:P] %*% What3[1:P]))*sw[i] 
  c.mean[i,4] <- inter4 + (gamma4*(X[i,1:P] %*% What4[1:P]))*sw[i] 
  c.mean[i,5] <- inter5 + (gamma5*(X[i,1:P] %*% What5[1:P]))*sw[i] 
  c.mean[i,6] <- inter6 + (gamma6*(X[i,1:P] %*% What6[1:P]))*sw[i] 
  c.mean[i,7] <- inter7 + (gamma7*(X[i,1:P] %*% What7[1:P]))*sw[i] 
  
  a.mean[i] <- interoverall + gammaall*(X[i,1:P] %*% Wtilde[1:P])*sw[i] + (C[i,1:Q] %*% delta[1:Q])
}
...
interoverall ~ dnorm(0, 1.0E-04)
gammaall ~ dnorm(0, 1.0E-04)
...
var.s <- c("Wtilde","What1","What2","What3","What4","What5","What6","What7",
           "gamma1","gamma2","gamma3","gamma4","gamma5","gamma6","gamma7","gammaall",
           "inter1","inter2","inter3","inter4","inter5","inter6","inter7","interoverall",
           "a.tau","c.tau")
